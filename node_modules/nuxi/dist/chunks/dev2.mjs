...
import { type } from 'os';
import { c as consola } from '../shared/nuxi.DSPrE7vg.mjs';
import 'node:util';
import 'node:path';
import 'node:process';
import 'node:tty';
...
import { l as loadNuxtManifest, w as writeNuxtManifest } from '../shared/nuxi.c8JNj5hu.mjs';
...
class NuxtDevServer extends EventEmitter$1 {
  ...
  async _renderError(res, _error) {
    ...
    const loadingTemplate = this.options.loadingTemplate || this._currentNuxt?.options.devServer.loadingTemplate || await importModule("@nuxt/ui-templates", this.options.cwd).then((r) => r.loading).catch(() => {
